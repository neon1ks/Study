    ...
    : QDialog(parent)
{
    ...
    for (int row = 0; row < tracks->count(); ++row) {
        ...
        auto *item0 = new QTableWidgetItem(track.title);
        tableWidget->setItem(row, 0, item0);

        auto *item1 = new QTableWidgetItem(QString::number(track.duration));
    ...
    QPushButton *addTrackButton = buttonBox->addButton(
            tr("&Add Track"), QDialogButtonBox::ActionRole);
