...
*/
    ...
    if (!numTicks.isNull()) {
        zoomStack[curZoom].scroll(numTicks.x(), numTicks.y());
    }
...
void Plotter::refreshPixmap()
{
    ...
    pixmap.fill(QPalette{}.color(QPalette::Dark));

    QPainter painter(&pixmap);
    // Вызов initFrom() устанавливает в рисовальщике перо, фон и шрифт такими же,
    // как для виджета Plotter. Функция устарела.
...
void Plotter::drawCurves(QPainter *painter)
{
    ...
    static const QColor colorForIds[COLOR_COUNT] = {
    ...
    while (i.hasNext()) {
        ...
        painter->setPen(colorForIds[uint(id) % COLOR_COUNT]);
