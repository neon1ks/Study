...
void MainWindow::goToCell()
{
	...
	if (dialog.exec())
	{
		...
		spreadsheet->setCurrentCell(QStringView{ str }.mid(1).toInt() - 1, str[0].unicode() - 'A');
	}
}
...
void MainWindow::sort()
{
	...
	if (dialog.exec())
	{
		...
		compare.params[0] = { dialog.getPrimaryColumnIndex(), dialog.getPrimaryAscending() };
		compare.params[1] = { dialog.getSecondaryColumnIndex(), dialog.getSecondaryAscending() };
		compare.params[2] = { dialog.getTertiaryColumnIndex(), dialog.getTertiaryAscending() };
...
void MainWindow::createActions()
{
	...
	findAction->setIcon(QIcon(":/icon/find.png"));
	findAction->setShortcut(QKeySequence::Find);
	findAction->setStatusTip(tr("Find a matching cell"));
	connect(findAction, SIGNAL(triggered()), this, SLOT(find()));

	goToCellAction = new QAction(tr("&Go to Cell..."), this);
	goToCellAction->setIcon(QIcon(":/icon/gotocell.png"));
...
{
	locationLabel = new QLabel(QString::fromUtf8(" W999 "));
...
auto MainWindow::okToContinue() -> bool
{
	if (isWindowModified())
	{
...
auto MainWindow::loadFile(const QString &fileName) -> bool
{
	if (!spreadsheet->readFile(fileName))
	{
		statusBar()->showMessage(tr("Loading canceled"), TIMEOUT);
		return false;
	}
...
auto MainWindow::saveFile(const QString &fileName) -> bool
{
	...
	{
		statusBar()->showMessage(tr("Saving canceled"), TIMEOUT);
		return false;
	}
...
auto MainWindow::strippedName(const QString &fullFileName) -> QString
{
