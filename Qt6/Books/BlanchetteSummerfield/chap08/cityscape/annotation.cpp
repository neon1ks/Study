...
Annotation::Annotation(const QString &text, bool major)
{
    ...
    // If `true` levelOfDetail always = 1
...
{
    qDebug() << "worldMatrixEnabled: " << painter->worldMatrixEnabled();

    qreal levelOfDetail = QStyleOptionGraphicsItem::levelOfDetailFromTransform(
            painter->worldTransform());
    qDebug() << "levelOfDetail: " << levelOfDetail;
    ...
    painter->drawText(
            rect.translated(+1, +1), str, QTextOption(Qt::AlignCenter));
