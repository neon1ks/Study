...
CityBlock::CityBlock(Kind kind)
{
    ...
    if (kind == Park) {
        color = QColor(192 + (std::rand() % 32), 255, 192 + (std::rand() % 16));
        shape.addRect(boundingRect());
    } else if (kind == SmallBuilding) {
        QRectF block(-7.5, -7.5, 15, 15);
        block.moveBottomLeft(
                QPointF((std::rand() % 6) - 3, (std::rand() % 6) - 3));
        shape.addRect(block);
        ...
        shape.addEllipse(boundingRect().adjusted(
        ...
        int padding1 = (std::rand() % 8) + 2;
        int padding2 = (std::rand() % 8) + 2;
        shape.addRect(boundingRect().adjusted(
                +padding1, +padding1, -padding2, -padding2));
        ...
        shape.addRect(QRectF(-16 + w1 + 4, -16 + (std::rand() % 4), w2, h2));
        ...
        shape.addRect(
                QRectF(-16 + w1 + 4, y + h2 + 4 + (std::rand() % 4), w3, h3));
    }
}
        ...
        const QStyleOptionGraphicsItem * /* option */, QWidget * /* widget */)
{
    if (painter->worldMatrixEnabled()) {
        ...
        if (levelOfDetail <= 4.0) {
            ...
            int coeff = 105 + static_cast<int>(levelOfDetail);
